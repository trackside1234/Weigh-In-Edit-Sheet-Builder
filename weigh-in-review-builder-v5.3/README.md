...
## v5 venue-code mapping

- Uses the supplied venue-based production code list as the primary lookup.
- Only Thoroughbred (`T`) entries are included. Harness and Greyhound entries are excluded.
- Unique venue mappings auto-fill the media code (for example Riccarton → `CBRY`, Te Rapa → `WAIK`, Rotorua → `ROTU`).
- Ambiguous international venues that appear under more than one production code are not guessed; the code field remains manually selectable.
- International Thoroughbred venue mappings are retained for future expansion, although the current meetings API query remains NZ Thoroughbred only.


## v5.1 UI update
- Edit-sheet silks/placings now match the VT Form Builder arrow presentation: no individual boxes, 14px silks, simple vertical rows.
- Display dates now explicitly use MONTH DAY format, e.g. `SEPTEMBER 7`.


## v5.2 UI update
- Enlarged silk display to 34px.
- Place getters are now rendered as a comma-separated inline list in the requested format:
  `WELL WRITTEN – 1ST, ALABAMA LASS – 2ND, MEDIA WORLD – 3RD, TARDELLI – 4TH, TUXEDO – 5TH`.


## v5.3 fix
- Restored a 34px silk beside every place getter.
- Each getter is formatted `HORSE – PLACE`, with comma separation.
- Removed the v5.2 single-silk treatment.
